...
const ProjectCard = ({ project, index }: { project: typeof projects[0], index: number }) => {
    ...
    return (
        ...
        >
            <Card
                noPadding
                className="h-full rounded-[2.6rem_2.2rem_2.9rem_2rem] border-[color:var(--stroke-subtle)] bg-[var(--surface)]"
            >
                <div className="grid h-full gap-6 p-5 md:p-7">
                    <div className={`relative min-h-[18rem] overflow-hidden ${mediaRadius}`}>
                        ...
                        <div className="absolute left-5 top-5 rounded-full border border-[color:var(--stroke-subtle)] bg-[rgb(20_27_24_/_0.55)] px-3 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.24em] text-[color:var(--foreground-subtle)] backdrop-blur-sm">
                            {project.category}
                        </div>
                    </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
                        <div className="space-y-4">
                            <p className="text-[0.68rem] font-semibold uppercase tracking-[0.34em] text-[color:var(--foreground-muted)]">
                                {project.note}
                            </p>
                            <h3 className="max-w-[10ch] text-4xl text-foreground sm:text-5xl">
                                {project.title}
                            </h3>
                            <p className="max-w-xl text-[0.77rem] uppercase tracking-[0.24em] text-[color:var(--foreground-muted)]">
                                {project.description}
                            </p>
                        </div>
                        ...
                        <motion.a
                            ...
                            transition={spring}
                            className="inline-flex h-14 items-center gap-3 rounded-full border border-[color:var(--stroke-subtle)] bg-[rgb(255_244_234_/_0.05)] px-6 text-sm font-semibold uppercase tracking-[0.18em] text-foreground"
                        >
